extends Node3D

@onready var gun_sprite = $CanvasLayer/Control/GunSprite
@onready var gun_rays = $GunRays
@onready var flash = preload("res://Player/Guns/muzzle_flash.tscn")
@onready var blood = preload("res://Particles/blood.tscn")
@onready var hit = preload("res://Particles/hit.tscn")
var damage = 1.5
var can_shoot: bool = true
var is_reloading: bool = false
var empty: bool = false

func _ready():
	gun_sprite.play("idle")
	enable_rays()
	
func enable_rays():
	for ray in gun_rays.get_children():
		if ray is RayCast3D:
			ray.enabled = true 
			ray.force_raycast_update()  
			
func check_hit():
	for ray in gun_rays.get_children():
		if ray is RayCast3D and ray.enabled and ray.is_colliding():
			var hit_object = ray.get_collider()
			if hit_object:
				#print("Hit:", hit_object.name)  
				if hit_object.is_in_group("Enemy"):
					var new_blood = blood.instantiate()
					get_tree().current_scene.add_child(new_blood)
					new_blood.global_transform.origin = ray.get_collision_point()
					new_blood.emitting = true
					hit_object.take_damage(damage)
					#print("Enemy hit!")
				elif !hit_object.is_in_group("Enemy"):
					var new_hit = hit.instantiate()
					get_tree().current_scene.add_child(new_hit)
					new_hit.global_transform.origin = ray.get_collision_point()
					new_hit.emitting = true
	
func make_flash():
	var f = flash.instantiate()
	add_child(f)
	
func _process(delta):
	
	if PlayerStats.ammo_shells == 0 and PlayerStats.ammo_shells_cur == 0:
		PlayerStats.shotgun_empty = true
	else:
		PlayerStats.shotgun_empty = false
	
	if PlayerStats.ammo_shells_cur == 0 and PlayerStats.ammo_shells >= 2 and !PlayerStats.reloading and !empty:
		empty = true
		gun_sprite.play("just_reload")
		await gun_sprite.animation_finished
		PlayerStats.ammo_shells_cur = 2
		PlayerStats.ammo_shells -= 2
		empty = false
	
	if Input.is_action_just_pressed("shoot") and can_shoot and PlayerStats.ammo_shells_cur >= 2:
		if PlayerStats.ammo_shells != 0:
			gun_sprite.play("shoot")
		else: 
			gun_sprite.play("just_shoot")
			
		make_flash()
		check_hit()
		can_shoot = false
		PlayerStats.reloading = true
		PlayerStats.change_shotgun_ammo(-2)
		await gun_sprite.animation_finished
		if PlayerStats.ammo_shells_cur <= 0:
			if PlayerStats.ammo_shells > 2:
				PlayerStats.ammo_shells_cur = 2
				PlayerStats.ammo_shells -= 2
			elif PlayerStats.ammo_shells == 2:
				PlayerStats.ammo_shells_cur = PlayerStats.ammo_shells
				PlayerStats.ammo_shells -= 2
		can_shoot = true
		PlayerStats.reloading = false
		
